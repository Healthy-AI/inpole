...
if __name__ == '__main__':
    ...
    if args.new_out_dir:
        if args.predict_only:
            raise ValueError(
                "The arguments '--new_out_dir' and '--predict_only' cannot be "
                "used together."
            )
    ...
    if config['experiment'] in ['ra', 'sepsis']:
        for subset in subsets:
            predict(
                config, pipeline, args.estimator, subset, metrics=metrics,
                switches_only=True
            )
    ...
    if args.estimator in ['sdt', 'rdt', 'truncated_rdt']:
        ...
        for subset in subsets:
            predict(
                config, pipeline, f'{args.estimator}_aligned', subset,
                metrics=metrics
            )
        ...
        for subset in subsets:
            predict(
                config, pipeline, f'{args.estimator}_pruned', subset,
                metrics=metrics
            )
